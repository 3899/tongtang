...
- **HomeKit · 专属桥**：每位成员勾选设备保存即自动获得自己的 HomeKit 桥（托管 HA 内置 homekit 集成，零额外组件），iPhone 家庭 App 扫码配对、Siri 直控；勾选与权限变化实时同步进家庭 App 且不破坏配对，权限收回自动消失；多 HA 按家各建一桥，摄像头默认 H.264 直通
